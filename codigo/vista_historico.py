...
def construir_excel_concordancia_dashboard(df_lote: pd.DataFrame) -> io.BytesIO:
    ...
    with pd.ExcelWriter(
        ...
        # - Se coloca debajo del “Top revisión” para no pisar bloques anteriores.
        # ---------------------------------------------------------
        row0 = top_r + 20
        ws2.write(row0, 0, "Estadísticas por biomarcador (lote)", fmt_title)

        # Cabecera
        for j, col in enumerate(stats_lote.columns):
            ws2.write(row0 + 1, j, col, fmt_header)

        # Filas
        for i, (_, r) in enumerate(stats_lote.iterrows(), start=0):
            for j, col in enumerate(stats_lote.columns):
                ws2.write(row0 + 2 + i, j, r[col])
