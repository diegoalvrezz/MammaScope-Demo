...
def mostrar_exploracion_sql():
    ...
    # ==================================================
    # Estadísticas globales acumuladas (BD completa)
    # ==================================================
    st.subheader("Estadísticas globales acumuladas (base de datos completa)")
    st.caption("Calcula métricas acumuladas sobre todos los casos emparejados guardados en la tabla `muestras`.")

    try:
        conn = sqlite3.connect(DB_PATH)

        # Se seleccionan columnas mínimas necesarias para construir métricas.
        # Esto reduce carga respecto a leer todo el registro completo.
        ...
        if df_all.empty:
            st.info("La tabla `muestras` está vacía. No hay estadísticas acumuladas.")
        else:
            # Se contabilizan muestras únicas por sample_id (clave operativa).
            st.write(f"Casos emparejados totales (únicos por identificador de muestra): {df_all['sample_id'].nunique()}")
            st.caption("Se cuenta el número de muestras únicas almacenadas (una por `sample_id`).")

            # Calcula TP/TN/FP/FN, % concordancia, kappa y McNemar, por biomarcador.
            # El cutoff de Ki67 IHQ se fija aquí para homogeneizar interpretación (20% habitual, si así lo has definido).
            stats_global = build_stats_table_from_df(df_all, ki67_cutoff_ihq=20.0)

            st.caption("Tabla resumen con métricas de concordancia y discordancia entre IHQ y MammaTyper®.")
            st.dataframe(stats_global, use_container_width=True)

            st.caption(
                "Interpretación rápida: si **McNemar_p < 0.05** y la **Tendencia** indica '+/-', "
                "hay asimetría consistente en discordancias entre IHQ y MammaTyper®."
            )

            # Descarga del resumen estadístico global en Excel para informes y anexos.
            ...
            with pd.ExcelWriter(
                ...
                stats_global.to_excel(writer, index=False, sheet_name="estadisticas_globales")
            ...
            st.download_button(
                ...
                key="sql_stats_globales_dl",
                help="Descarga la tabla de métricas globales acumuladas en formato Excel."
            )

    except Exception as e:
        st.error(f"Error calculando estadísticas globales: {e}")
...
def main():
    ...
    render_login_gate("TFG MammaTyper – Integración Patwin + MammaTyper")
    ...
    if require_role("jefe"):
        opciones += ["Explorar base de datos histórica (SQL)", "Bases no cruzadas"]
    ...
    # =========================
        ...
        # Si no hay archivos, se corta para evitar estados incoherentes.
        ...
        # Explica el pipeline para transparencia y para que sea defendible en memoria/TFG.
        st.markdown(
            "En este paso se:\n"
        ...
        # Botón único para evitar ejecutar el pipeline automáticamente.
        if st.button("Procesar y guardar muestras en BD"):
            u = current_user() or {}
            log_action(u.get("username"), "process_click", None)

            # Validación de formatos antes de intentar extraer (evita errores y false positives).
            ...
            st.success("Excel y PDF con formato correcto. Iniciando extracción...")

            try:
                # -------------------------
                # Extracción Excel
                # -------------------------
                # Se pasa a BytesIO para que las funciones trabajen como si fuese un archivo.
                excel_file = io.BytesIO(st.session_state["excel_bytes"])
                registros_excel = extraer_registros_patwin(excel_file)

                # Índice por sample_id para emparejar rápido desde el PDF.
                # sample_id es la clave operativa de emparejamiento.
                idx_excel = {
                ...
                # -------------------------
                # Fusión por sample_id
                # -------------------------
                # Estrategia:
                # - Se recorre el PDF y se busca su pareja en Excel.
                # - Solo se guarda en tabla principal cuando hay combinación Excel+PDF.
                # - Si falta la pareja, se registra como “sin match” con origen PDF.
                for reg_pdf in regs_pdf:
                    ...
                    # Construye el registro combinado final con reglas de fusión definidas en extraccion.py.
                    combinado = fusionar_registro_patwin_pdf(reg_excel, reg_pdf)

                    # Inserta el registro en la tabla principal `muestras`.
                ...
                # Excel sin PDF
                # -------------------------
                # Los sample_id del Excel que no aparecen en el PDF se registran como no cruzados con origen EXCEL.
                ...
                # -------------------------
                # Guardado en sesión para Paso 3
                # -------------------------
                st.session_state["ultimo_lote"] = procesados

                # Identifica muestras con aviso clínico/discordancia para alertar al usuario.
                # Esta alerta no sustituye revisión: es una señal de “revisar”.
                ...
                if ids_con_aviso:
                    st.warning(
                        "Aviso para muestras: "
                        + ", ".join(ids_con_aviso)
                        + ". IMPORTANTE: Aviso automático: indica discordancias potenciales y requiere revisión."
                    )

                # Resumen del procesamiento del lote para mostrar en Paso 3.
                resumen = {
                ...
                # Auditoría
                # -------------------------
                # Se registra el evento y tamaños para trazabilidad operativa.
                log_action(
                    u.get("username"),
                    "process_done",
                    {
                ...
                # -------------------------
                st.success(
                    f"Proceso finalizado. {len(procesados)} muestras combinadas guardadas en la base de datos."
                )
        ...
        # Navegación al finalizar Paso 2 (se permite ir atrás o avanzar).
        st.markdown("---")
